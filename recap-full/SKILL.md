---
name: recap-full
description: Recap COMPLET de TOUTE la session en cours, structure narrative item par item, en langage humain pour Florent (qui n'est pas dev). Invoquer quand Florent dit "/recap-full", "recap complet", "fais-moi le bilan complet de la session", "resume toute la session", "recap session entiere". Pas de commit, pas de push, pas de KB — juste un message clair dans le chat couvrant tous les sujets abordes. Pour un recap COURT du sujet en cours uniquement (PB + fix en quelques lignes), utiliser /recap.
trigger: user-invocable — /recap-full
scope: global — tout projet
---

# /recap-full — Recap complet de la session en cours

**But** : repondre a "fais-moi le bilan complet de la session" en couvrant TOUS les sujets abordes depuis le debut de la conversation, en langage humain, sans jargon, sans code, sans hash.

Ce skill ne fait QUE le recap complet. Il ne commite pas, ne push pas, ne touche pas la KB, n'ecrit pas de handoff. **Juste un message clair dans le chat.**

**Difference avec `/recap`** : `/recap` = bref, sujet en cours uniquement (1 paragraphe PB + 1 paragraphe fix). `/recap-full` = couvre toute la session, structure narrative complete par item.
